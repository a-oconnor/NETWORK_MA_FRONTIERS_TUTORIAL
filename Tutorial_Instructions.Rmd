...
packages <- c("dplyr", "network", "MASS", "rjags", "EcoSimR", "miceadds", "openxlsx", "DT", "mcmcplots")
...
## The Mapping File
The first dataset is a mapping file called "map_txt.csv" which contains the list of all treatments in the dataset and maps each treatment to a number starting from 1. This number, rather than the character-based name, is subsequently used in several scripts. When setting up the map_txt file, it is essential that the baseline treatment is labeled as 1. In our "map_txt.csv", the baseline treatment is the placebo group, therefore the placebo group has number 1. Make sure your "map_txt.csv" file has the same column names as this one because the subsequent code uses exactly these column names. The "map_txt.csv" file has 3 columns of data. 
...
MTCdata <- read.csv(file = "./data/MTCdata.csv", stringsAsFactors = F)
head(MTCdata,3)
...
### Contrast-Level Data
It is also possible that the extracted data are contrast-level data. Contrast level describe the comparison of a pair of treatments. An example of a contrast-level data for catagorical outcomes is the odds ratio. For  continous data, an example of a contreact level outcome is the mean difference.  Contrast-level data are extracted because  authors do not report the arm level data or if the reviewer has a preference for the contrast level data. In veterinary clincial trials a common reason for using a contrast-level effect is that the trial was conducted in a setting that required adjusttment for non-independent observataions due to clustering by pen or farm. 
...
MTCdata_contrast <- read.csv(file = "./data/MTCdata_contrast.csv", stringsAsFactors = F)
...
In our workflow of conducting a network meta-anlaysis as described in the Frontiers mansucripts one of the first steps is to create the posterior distribtion of the baseline treatment on the log odds scale. This is achieved with the getBaselinePrior.R function. This functions determiens the hyperprior value for the baseline treatment on the logit scale by seperate mcmc simulation. 
...
* The dataset to be used -- In our example this is the MTCdata  
* The option for dataType- Arm or Contrast-- In our example - Arm 
* The option for good_event: Indicator, 1 if the events are good, 0 if the events are bad,. If the the event in your dataset is bad event, then you can set good_event to be 0, otherwise 1. This option will affect the rank of the treatments. 
...
* d[#D]: The logs odds ratio of the basic paramters with the baseline as number of treatment as the denominator  
* lor[#N,#D]: The log odds ratio of basic and funcational paramters with the numerator treatment (#N) divided by the denominator treatment (#D). 
...
* rka[#]: The rankings for the treatments


```{r,run-general-model-smry,message=F,results='hide'}
jags_output <- general_model(MTCdata = MTCdata, baselinePriorSmry = baselinePriorSmry,good_event = 0, dataType = "Arm")
...
## Check the goodness of fit using the residdual 
...
Defaults that are rarely changed, but can be modified in the "direct_comparison.R" script in the scripts folder.
...
For example, the cell [1,2] is the posterior median of the risk ratio of treatment B to treatment A, and cell [2,1] is the corresponding 95% credible interval.

If you want to show the odds ratios and their 95% credible interval:

```{r, message=F}
MTC_comp_OR <- MTC_comparison(map_txt = map_txt, model_smry = general_model_smry, measure = "OR",
                              stat = "median", save_res = T)
MTC_comp_OR
```

As for risk ratios and their 95% credible interval, 
...
```{r, message=F}
mean_rank <- mean_rank <- mean_rank_matrix(map_txt = map_txt, model_smry = general_model_smry, arm_name = "abbr",
                              save_res = T)
mean_rank
```
...
prob_rank <- prob_rank_table(jags.out = jags.out, map_txt = map_txt, good_event = 0, arm_name = "abbr",save_res = T)
prob_rank
```


For example, the cell [1,2] in 'prob_comp' element is the probability that Treatment A is better than Treatment B. cell [2,1] is the verse. Column 'Rank_1_prob' in 'rank_smry' component is the probability that each treatment can be ranked top 1.
...
ranking_plot(MTCdata = MTCdata, map_txt = map_txt, model_smry = general_model_smry, arm_name = "abbr", cex = 2)
```
